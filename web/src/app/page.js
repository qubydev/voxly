...
export default function Dashboard() {
  ...
  const [jobState, setJobState] = useState({ status: 'idle', finishedChunks: 0, totalChunks: 0 })
  const [isDownloading, setIsDownloading] = useState(false)
  const [isInitializing, setIsInitializing] = useState(true)

  const pollStatus = async (isFirstCheck = false) => {
    try {
      const res = await fetch('/api/tts/status')
      if (!res.ok) {
        if (isFirstCheck) setIsInitializing(false)
        return
      }
      ...
      if (data.status === 'pending' || data.status === 'processing') {
        setGenerating(true)
        setTimeout(() => pollStatus(false), 1000)
      } else {
        setGenerating(false)
        if (data.status === 'completed' && data.audioUrl) {
          setAudioUrl(data.audioUrl)
        } else if (data.status === 'failed' && !isFirstCheck) {
  ...
  useEffect(() => {
    ...
    if (savedVoice) setSelectedVoice(JSON.parse(savedVoice))
  ...
  useEffect(() => {
    if (!isPending && !session) router.push('/landing')
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true)
    setAudioUrl(null)
    setJobState({ status: 'pending', finishedChunks: 0, totalChunks: 0 })
        ...
        <div className="flex-1 flex flex-col min-h-0 min-w-0">
          <div className="flex-1 min-h-0 px-6 md:px-10 py-8">
            <textarea
              ...
              value={text}
              disabled={generating || isInitializing}
          ...
          <div className="border-t border-border px-5 py-4 flex flex-col gap-3 shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                disabled={generating || isInitializing || !text.trim() || isOverLimit}
      ...
      <Drawer open={mobileSettingsOpen} onOpenChange={setMobileSettingsOpen}>
        <DrawerContent className="rounded-none">
          ...
          <div className="overflow-y-auto px-5 py-5 flex flex-col gap-6 pb-10">
            <VoiceSelector selected={selectedVoice} onSelect={v => { setSelectedVoice(v) }} />
            <TTSSettings settings={settings} onChange={setSettings} />
          </div>
        </DrawerContent>
      </Drawer>
